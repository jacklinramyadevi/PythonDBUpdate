...
class SPD_INFO():
    ...
    def insert_card_type(self,bankId,creditID,creditType,creditName,urlPic,baseInfo,privillageInfo,creditcarddetail):
        cursor = self.conn.cursor()

        datetimenowTime=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        sql = 'insert into CREDIT_TYPE_INFO (BANK_ID, CARD_TYPE_CODE,CREDIT_CARD_TYPE,CREDIT_CARD_NAME,CREDIT_CARD_PIC,CREDIT_BASE_INFO,CREDIT_PRIVILEGE_INFO,STATUS,CRT_TM,LMT_TM,CREDIT_CARD_DETAIL) values({bankId},\'{creditID}\',\'{creditType}\',\'{creditName}\',\'{urlPic}\',\'{baseInfo}\',\'{privillageInfo}\',\'{status}\',\'{crtTime}\',\'{lmtTime}\',\'{creditcarddetail}\')'.format(bankId=bankId  \
        ,creditID=creditID,creditType=creditType,creditName=creditName,urlPic=urlPic,baseInfo=baseInfo,privillageInfo=privillageInfo,status=1, crtTime=datetimenowTime,lmtTime=datetimenowTime,creditcarddetail=creditcarddetail)
                        ...
                        for rowi in range(len(self.jitem)):
                            ...
                            if len(cnn)!=0 and len(rnn) != 0:
                                if pnn in province and cnn in cityName and rnn in areaName:
                                    ...
                                    self.insert_pcr_item('SPD',pName,str(pId),cityName,str(cityId),areaName,str(areaId),cityCode)
                                    break


    def load_support_city(self,filename):

        self.delete_bank_city_info('SPD')
        ...
        with open(filename,'r',encoding='UTF-8') as f:
            ...
            for p in range(len(jstr)):
                ...
                for citem in range(len(self.SPD_card_list)):
                    cmb_index = cmb_index + 1
                    bankCardCode = self.SPD_card_list[citem]['creditID']
                    self.insert_bank_city(bankCardCode,'SPD_'+str(cmb_index),city_item['provinceName'],city_item['cityName'],city_item['areaName'])
    ...
    def load_card_type(self,filename):
        ...
        with open(filename,'r',encoding='UTF-8') as f:
            ...
            for p in range(len(jstr)):
                ...
                self.SPD_card_list.append(card_type_item)
                creditcarddetail = jstr[p]['otherParams']
                self.insert_card_type(self.bankId,creditID,creditType,creditName,urlPic,baseInfo,privillageInfo,creditcarddetail)
                
            #print(self.SPD_card_list)
...
if __name__ == '__main__':
    ...
    spd.load_card_type('F:\\Jacklin\\creditcard\\DB\\spd_card_list.json')#Finished
    #spd.load_support_city('F:\\Jacklin\\creditcard\\DB\\spd_support_city.json')

    #spd.load_pt_city('F:\\Jacklin\\creditcard\\DB\\pt_city_info.json')#Finished

    #spd.load_pcr_info('F:\\Jacklin\\creditcard\\DB\\SPD_pcr_info.json')#Finished

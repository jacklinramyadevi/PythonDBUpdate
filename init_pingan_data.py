...
class PINGAN_INFO():
    ...
    def __init__(self):
        self.conn = None
        self.pingan_card_list = []
        self.bankCode = 'PINGAN'
        self.bankId = 10
    ...
    def insert_card_type(self,bankId,creditID,creditType,creditName,urlPic,baseInfo,privillageInfo,producttype):
        cursor = self.conn.cursor()

        datetimenowTime=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        sql = 'insert into CREDIT_TYPE_INFO (BANK_ID, CARD_TYPE_CODE,CREDIT_CARD_TYPE,CREDIT_CARD_NAME,CREDIT_CARD_PIC,CREDIT_BASE_INFO,CREDIT_PRIVILEGE_INFO,STATUS,CRT_TM,LMT_TM,CREDIT_PRODUCT_TYPE) values({bankId},\'{creditID}\',\'{creditType}\',\'{creditName}\',\'{urlPic}\',\'{baseInfo}\',\'{privillageInfo}\',\'{status}\',\'{crtTime}\',\'{lmtTime}\',\'{product_type}\')'.format(bankId=bankId  \
        ,creditID=creditID,creditType=creditType,creditName=creditName,urlPic=urlPic,baseInfo=baseInfo,privillageInfo=privillageInfo,status=1, crtTime=datetimenowTime,lmtTime=datetimenowTime, product_type=producttype)
        ...
        with open(filename,'r',encoding='UTF-8') as f:
            ...
            for p in range(len(jstr)):
                ...
                for ci in range(len(pCity)):
                    ...
                    cregion = pCity[ci]['district']
                    r_index = 0
                    #print('cregion ')
                    #print(len(cregion))
                    for ri in range(len(cregion)):
                        areaName = cregion[ri]['districtName']
                        areaId = cregion[ri]['selfCode']
                        ...
                        for rowi in range(len(self.jitem)):
                            ...
                            if len(cnn)!=0 and len(rnn) != 0:
                                ...
                                if pnn in province and cnn in cityName and rnn in areaName:
                                    ...
                                    self.insert_pcr_item('PINGAN',pName,str(pId),cityName,str(cityId),areaName,str(areaId),cityCode)
                                    break


    def load_support_city(self,filename):

        self.delete_bank_city_info('PINGAN')
        ...
        with open(filename,'r',encoding='UTF-8') as f:
            ...
            for p in range(len(jstr)):
                ...
                for citem in range(len(self.pingan_card_list)):
                    cmb_index = cmb_index + 1
                    bankCardCode = self.pingan_card_list[citem]['creditID']
                    self.insert_bank_city(bankCardCode,'PINGAN_'+str(cmb_index),PINGAN_item['provinceName'],PINGAN_item['cityName'],PINGAN_item['areaName'])
    ...
    def load_card_type(self,filename):
        ...
        with open(filename,'r',encoding='UTF-8') as f:
            ...
            for p in range(len(jstr)):
                ...
                self.pingan_card_list.append(card_type_item)
                productType = jstr[p]['productType']
                
                self.insert_card_type(self.bankId,creditID,creditType,creditName,urlPic,baseInfo,privillageInfo,productType)
                
            print(self.pingan_card_list)
...
if __name__ == '__main__':
    ...
    pingan.load_card_type('F:\\Jacklin\\creditcard\\DB\\PAB_card_list.json')#Finished

    #pingan.load_pt_city('F:\\Jacklin\\creditcard\\DB\\pt_city_info.json')#Finished

    #pingan.load_pcr_info('F:\\Jacklin\\creditcard\\DB\\pingan_pcr_info.json')#Finished

    #pingan.load_support_city('F:\\Jacklin\\creditcard\\DB\\pingan_support_city.json')
